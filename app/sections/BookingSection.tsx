...
export default function BookingSection() {
  ...
  const { state, provinces, cities, selectedProvince, setProvince, destinations, selectedDestination, tripDetails, detailsLoading, aiInterests, aiDietary, activeAddOns, prefsLoading, aiLoading, confirmTrip, setTrip, setPrefs, toggleAddOn, setContact, setErrors, submit, reset, total } = useBookingForm('international');
          ...
          <motion.div className="booking-tags" variants={stagger}>
            {detailsLoading ? (
              ...
              <>
                ...
                <motion.span className="booking-tag" variants={chipPop}>{tripDetails.transportType === 'highspeed-rail' ? '🚄 高铁商务座' : '✈️ 航班公务舱'}</motion.span>
                <motion.span className="booking-tag" variants={chipPop}>全程奢华五星酒店</motion.span>
              </>
            ) : (
              <>
                <motion.span className="booking-tag" variants={chipPop}>{tripConfig.days}天{tripConfig.days - 1}晚</motion.span>
                <motion.span className="booking-tag" variants={chipPop}>全程奢华五星酒店</motion.span>
                <motion.span className="booking-tag" variants={chipPop}>私人公务机接驳</motion.span>
              </>
            )}
          </motion.div>
          ...
          <div className="booking-pref-group" data-field="preferences.interests">
            ...
            {prefsLoading ? (
              ...
              <motion.div className="booking-chip-grid" variants={stagger}>
                {(aiInterests.length > 0 ? aiInterests : INTERESTS).map((item) => (
          ...
          <div className="booking-pref-group">
            ...
            {prefsLoading ? (
              ...
              <motion.div className="booking-chip-row" variants={stagger}>
                {(aiDietary.length > 0 ? aiDietary : DIETARY_OPTIONS).map((opt) => (
